...
router.patch('/:project_id/:task_id', auth, async (req, res) => {
    try {
        ...
        if (index > -1) {
            ...
            var pr = await Project.findOneAndUpdate({ _id: req.params.project_id }, { name: req.body.name, comments: commentsArr, tasks: taskArr, $push: { alteration: { user: mongoose.Types.ObjectId(req.user.id), editType: req.body.editType, documentRef: mongoose.Types.ObjectId(req.params.task_id) } } }, {new: true});
